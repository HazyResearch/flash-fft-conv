...
~81% accuracy on the CIFAR10 val set.

Each epoch takes approximately 43s on an H100 GPU.
'''
import torch
...
from flashfftconv_long_convs import LongConvModel
...
valloader = torch.utils.data.DataLoader(
    valset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, drop_last=True)
testloader = torch.utils.data.DataLoader(
    testset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, drop_last=True)
...
def eval(epoch, dataloader, checkpoint=False):
    ...
    if checkpoint:
        ...
        return acc, eval_loss/(batch_idx + 1)
...
for epoch in pbar:
    if epoch == 0:
        ...
        pbar.set_description('Epoch: %d | Val acc: %1.3f | Val loss: %1.3f' % (epoch, val_acc, val_loss))
    train()
    val_acc, val_loss = eval(epoch, valloader, checkpoint=True)
    eval(epoch, testloader)
    print()
    print('Epoch: %d | Val acc: %1.3f | Val loss: %1.3f' % (epoch, val_acc, val_loss))
    print()
    scheduler.step()
