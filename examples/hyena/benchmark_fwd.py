...
def benchmark(config):
    ...
    model = SequenceLightningModule(config).cuda()

    L = config.dataset.max_length
    B = config.batch_size
    ...
    # warmup
    model.model.backbone(input_ids)
    ...
    with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=dtype == 'half'):
        _, ret = benchmark_forward(model.model.backbone, input_ids, repeats=repeats, verbose=True, amp_dtype=torch.float16, amp=dtype == 'half')
