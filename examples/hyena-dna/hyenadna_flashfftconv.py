...
class HyenaFilter(OptimModule):
    def __init__(
        ...
        filter = torch.randn(1, seq_len, d_model) * 0.001
        filter = self.modulation(t, filter)
        filter = filter.transpose(-1, -2).squeeze(0).contiguous()
        self.register_buffer('k_ones', filter)
...
class HyenaOperator(nn.Module):
    def __init__(
        ...
        self.flash_short_filter = FlashDepthWiseConv1d(
            inner_width,
            3,
            padding=1,
    ...
    def forward(self, u, *args, **kwargs):
        ...
        uc = self.flash_short_filter(u)[...,:l]
        x1, x2, v = uc.split(self.d_model, dim=1)
        k = self.filter_fn.filter(l_filter) # assume inference mode
